...
class SGTensor{
public:
    SGTensor(bm_handle_t handle, const char *name, float scale, bm_tensor_t* tensor, bool relase):
...
using TensorPtr = std::shared_ptr<SGTensor>;
using TensorVec = std::vector<TensorPtr>;

class SGNetwork : public Uncopiable {
...
public:
    SGNetwork(void *bmrt, const std::string& name);
    void showInfo();
