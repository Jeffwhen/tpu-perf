...
SGNetwork::SGNetwork(void *bmrt, const std::string &name): m_bmrt(bmrt), bmodelPath(name) {
    m_handle = static_cast<bm_handle_t>(bmrt_get_bm_handle(bmrt));
    if (!bmrt_load_bmodel(m_bmrt, bmodelPath.c_str())) {
        SGLOG(FATAL, "load bmodel(%s) failed!", bmodelPath.c_str());
    }
    const char **names;
    int num;
...
void SGNetwork::showInfo()
{
    ...
    for(int i=0; i<m_netinfo->output_num; i++){
        ...
        SGLOG(INFO, "  Output %d) '%s' shape=%s dtype=%s scale=%g",
              i,
...
TensorVec SGNetwork::createOutputTensors(){
    TensorVec tensors;
    ...
    for(int i = 0; i < m_netinfo->output_num; ++i) {
        ...
        tensors.push_back(std::make_shared<SGTensor>(m_handle, m_netinfo->output_names[i],
                                                    m_netinfo->output_scales[i], &innerTensors[i], i==0));
    }
    return tensors;
}

TensorVec SGNetwork::createInputTensors(){
    TensorVec tensors;
    ...
    for(int i = 0; i < m_netinfo->input_num; ++i) {
        ...
        innerTensors[i].st_mode = BM_STORE_1N;
        innerTensors[i].device_mem = bm_mem_null();
        tensors.push_back(std::make_shared<SGTensor>(m_handle, m_netinfo->input_names[i],
                                                    m_netinfo->input_scales[i], &innerTensors[i], i==0));
    }
    return tensors;
}

int SGNetwork::forward(TensorVec inTensors, TensorVec outTensors) {
    ...
    if (!m_netinfo->is_dynamic && runtime_batch_size < static_batch_size)
    {
        ...
        SGLOG(DEBUG, "override batch size from %d to %d", runtime_batch_size, static_batch_size);
        for (int i = 0; i < m_netinfo->input_num; ++i)
    ...
    for(int i = 0;i < m_netinfo->output_num; ++i) {
        ...
        SGLOG(INFO, "output_tensor [%d] size=%d", i, bmrt_tensor_device_size(&tensor));
    }
#endif
...
size_t SGTensor::get_dtype_len() const {
    ...
    if(t == BM_FLOAT32 || t == BM_INT32 || t== BM_UINT32){
        ...
        SGLOG(FATAL, "Not support dtype=%d", t);
        return 0;
    }
}

ssize_t SGTensor::partial_shape_count(size_t begin, size_t end)
{
    size_t count = 1;
...
size_t SGTensor::get_mem_size() const
{
...
unsigned char *SGTensor::get_raw_data() {
...
float *SGTensor::get_float_data() {
        if (m_tensor->dtype == BM_FLOAT32) {
            ...
            SGLOG(FATAL, "NOT support dtype=%d", m_tensor->dtype);
        }
        return m_float_data;
}

size_t SGTensor::fill_host_mem(void *ptr, size_t len)
{
    auto bytes = get_mem_size();
...
void SGTensor::dumpData(const char* filename) {
    if (m_tensor->dtype == BM_FLOAT32) {
