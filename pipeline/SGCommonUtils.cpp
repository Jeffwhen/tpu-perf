...
void TimeRecorder::show() const
{
    ...
    for(size_t i=0; i<eventNum; i++){
        SGLOG(INFO, "%d: %s: %g ms", i, events[i].c_str(), timepoints[i]/1000.0);
        if(i!=eventNum-1){
            SGLOG(INFO, "  | %g ms", (timepoints[i+1]-timepoints[i])/1000.0);
        } else {
            SGLOG(INFO, "  | %g ms", (endTime-timepoints[i])/1000.0);
        }
    }
    SGLOG(INFO, "%d: %s: %g ms", eventNum, "==end==", endTime/1000.0);
}
...
std::string escape(const std::string &text){
    ...
    while(pos<text.size()){
        ...
        if((c&0x80) == 0) {
            ...
            SGLOG(WARNING, "'%s' cannot be encoded to utf8", text.c_str());
            return encodeStr;
        }
